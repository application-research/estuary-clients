...
class CollectionsApi
{
     ...
     *
     * @param  string $coluuid coluuid (required)
     * @param  int[] $content_ids Content IDs to add to collection (required)
     *
     * @throws \Swagger\Client\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return string
     */
    public function collectionsColuuidPost($coluuid, $content_ids)
    {
        list($response) = $this->collectionsColuuidPostWithHttpInfo($coluuid, $content_ids);
        return $response;
    }
     ...
     * @return array of string, HTTP status code, HTTP response headers (array of strings)
     */
    public function collectionsColuuidPostWithHttpInfo($coluuid, $content_ids)
    {
        $returnType = 'string';
        $request = $this->collectionsColuuidPostRequest($coluuid, $content_ids);
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function collectionsColuuidPostAsync($coluuid, $content_ids)
    {
        return $this->collectionsColuuidPostAsyncWithHttpInfo($coluuid, $content_ids)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function collectionsColuuidPostAsyncWithHttpInfo($coluuid, $content_ids)
    {
        $returnType = 'string';
        $request = $this->collectionsColuuidPostRequest($coluuid, $content_ids);
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    protected function collectionsColuuidPostRequest($coluuid, $content_ids)
    {
        // verify the required parameter 'coluuid' is set
        if ($coluuid === null || (is_array($coluuid) && count($coluuid) === 0)) {
            throw new \InvalidArgumentException(
        ...
        if ($content_ids === null || (is_array($content_ids) && count($content_ids) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $content_ids when calling collectionsColuuidPost'
            );
        }
