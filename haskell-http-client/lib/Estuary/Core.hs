...
-- | 
data EstuaryConfig = EstuaryConfig
...
-- | display the config
instance P.Show EstuaryConfig where
  show c =
    T.printf
...
-- | constructs a default EstuaryConfig
--
-- configHost:
--
...
-- @"estuary/0.1.0.0"@
--
newConfig :: IO EstuaryConfig
newConfig = do
    logCxt <- initLogContext
    return $ EstuaryConfig
        { configHost = "https://api.estuary.tech"
        , configUserAgent = "estuary/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => EstuaryConfig -> auth -> EstuaryConfig
addAuthMethod config@EstuaryConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: EstuaryConfig -> IO EstuaryConfig
...
withStderrLogging :: EstuaryConfig -> IO EstuaryConfig
...
withNoLogging :: EstuaryConfig -> EstuaryConfig
...
data EstuaryRequest req contentType res accept = EstuaryRequest
  { rMethod  :: NH.Method   -- ^ Method of EstuaryRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of EstuaryRequest
  , rParams   :: Params -- ^ params of EstuaryRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (EstuaryRequest req contentType res accept) NH.Method
rMethodL f EstuaryRequest{..} = (\rMethod -> EstuaryRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (EstuaryRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f EstuaryRequest{..} = (\rUrlPath -> EstuaryRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (EstuaryRequest req contentType res accept) Params
rParamsL f EstuaryRequest{..} = (\rParams -> EstuaryRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (EstuaryRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f EstuaryRequest{..} = (\rAuthTypes -> EstuaryRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => EstuaryRequest req contentType res accept -> param -> EstuaryRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: EstuaryRequest req contentType res accept -> param -> EstuaryRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: EstuaryRequest req contentType res accept -> param -> EstuaryRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
-- ** EstuaryRequest Utils

_mkRequest :: NH.Method -- ^ Method 
          -> [BCL.ByteString] -- ^ Endpoint
          -> EstuaryRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = EstuaryRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader :: EstuaryRequest req contentType res accept -> [NH.Header] -> EstuaryRequest req contentType res accept
setHeader req header =
  req `removeHeader` P.fmap P.fst header &
  L.over (rParamsL . paramsHeadersL) (header P.++)

removeHeader :: EstuaryRequest req contentType res accept -> [NH.HeaderName] -> EstuaryRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => EstuaryRequest req contentType res accept -> EstuaryRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of 
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => EstuaryRequest req contentType res accept -> EstuaryRequest req contentType res accept
_setAcceptHeader req =
    case mimeType (P.Proxy :: P.Proxy accept) of 
        Just m -> req `setHeader` [("accept", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["accept"]

setQuery :: EstuaryRequest req contentType res accept -> [NH.QueryItem] -> EstuaryRequest req contentType res accept
setQuery req query = 
  req &
  L.over
...
addForm :: EstuaryRequest req contentType res accept -> WH.Form -> EstuaryRequest req contentType res accept
addForm req newform = 
...
_addMultiFormPart :: EstuaryRequest req contentType res accept -> NH.Part -> EstuaryRequest req contentType res accept
...
_setBodyBS :: EstuaryRequest req contentType res accept -> B.ByteString -> EstuaryRequest req contentType res accept
_setBodyBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: EstuaryRequest req contentType res accept -> BL.ByteString -> EstuaryRequest req contentType res accept
_setBodyLBS req body = 
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => EstuaryRequest req contentType res accept -> P.Proxy authMethod -> EstuaryRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    :: EstuaryConfig
    -> a
    -> EstuaryRequest req contentType res accept
    -> IO (EstuaryRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> EstuaryConfig
  -> IO (EstuaryRequest req contentType res accept)
_applyAuthMethods req config@(EstuaryConfig {configAuthMethods = as}) =
  foldlM go req as
  where
